...
An ordinary `{ ... }` branch of a value-producing `on` is a statement block and
has type `void`. Write the result expression directly, or use an expression
block `${ ... }` and supply its result with `break <value>`.
