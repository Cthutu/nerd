# Windows validation run

Platform: Windows-11-10.0.26200-SP0
Commit at start: `6bbc17e8e14facc95caee55abdca35db216bbb05`
Branch: `experiment/task-scheduler-performance`

Selection: ['build-debug', 'build-release']
Automated status: **passed**. Manual checks: see MANUAL.md.

| Stage | Status | Seconds | Log / reason |
| --- | --- | ---: | --- |
| build-debug | passed | 5.093 | [build-debug.log](build-debug.log) |
| build-release | passed | 7.673 | [build-release.log](build-release.log) |
