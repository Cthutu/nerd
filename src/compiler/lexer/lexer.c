...
internal bool lexer_lex_decimal_separator_digits(NerdSource source,
                                                 string     source_code,
                                                 usize      start,
                                                 usize*     io_index)
{
    ...
    while (i < source_code.count) {
        ...
        if (!lexer_is_decimal_digit(source_code.data[i])) {
            if (lexer_is_number_tail(source_code.data[i])) {
                return error_0103_invalid_number_literal(
                    source,
                    (ErrorSpan){.start = start, .end = i + 1},
                                               ...
                                               Lexer*     lexer)
{
    ...
    while (i < source_code.count) {
        ...
        if (bytes >= 8) {
            return error_0108_packed_integer_literal_too_large(
                source, (ErrorSpan){.start = start, .end = i});
        }
    ...
    array_push(lexer->integers, value);
    *io_index = i;
    return true;
}
                                  ...
                                  bool       allow_interpolation_start)
{
    ...
    if (lexer_is_decimal_digit(c)) {
        ...
        UNUSED(had_separator);

        bool is_float = base == 10 && i + 1 < source_code.count &&
                        source_code.data[i] == '.' &&
                        (lexer_is_decimal_digit(source_code.data[i + 1]) ||
                         source_code.data[i + 1] == '_');
        if (is_float) {
            i++;
            if (!lexer_lex_decimal_separator_digits(
                    source, source_code, start, &i)) {
                return false;
            }
            ...
            usize sanitized_len = 0;
            for (usize j = start; j < i; ++j) {
                if (source_code.data[j] != '_') {
                    buffer[sanitized_len++] = (char)source_code.data[j];
                }
            }
    ...
    if (c == '\'') {
        return lexer_lex_packed_integer_literal(
            source, source_code, io_index, lexer);
    }
    ...
    if (c == '.') {
        if (i + 2 < source_code.count && source_code.data[i + 1] == '.' &&
            ...
            array_push(lexer->tokens,
                       (Token){.kind = TK_Ellipsis, .offset = (u32)i});
            *io_index = i + 3;
...
#    pragma GCC diagnostic ignored "-Winitializer-overrides"
    ...
    static TokenKind token_lookup[128] = {
        ...
        ['?'] = TK_Question,  ['\\'] = TK_Backslash, ['&'] = TK_Amp,
...
usize lex_token_end_offset(const Lexer* lexer, const Token* token)
{
    ...
    case TK_Integer:
        {
            if (lexer->source.source.count > token->offset &&
    ...
    case TK_Float:
        {
            ...
            if (index < lexer->source.source.count &&
