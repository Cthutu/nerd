                                                   ...
                                                   AstToken             next)
{
    ...
    if (next.kind == TK_String || next.kind == TK_CString ||
...
internal bool ast_emit_interpolated_string_continuation(AstParseState* state,
                                                        AstToken       token,
                                                        u32  left_node,
                                                        u32  right_node,
                                                        u32* out_node)
{
    AstKind left_kind        = state->nodes[left_node].kind;
    u32     left_token_index = state->nodes[left_node].token_index;
    u32     left_a           = state->nodes[left_node].a;
    u32     left_b           = state->nodes[left_node].b;
    AstNode right_copy       = state->nodes[right_node];
    ASSERT(left_kind == AK_InterpolatedString,
           "Expected interpolated string on lhs");
    UNUSED(left_kind);

    u32 part_start = (u32)array_count(state->nodes);
    for (u32 i = left_a; i < left_b; ++i) {
        if (!ast_emit_node(state, state->nodes[i], NULL)) {
            return false;
        }
    }

    if (right_copy.kind == AK_StringLiteral) {
        if (!ast_emit_node(state, right_copy, NULL)) {
            return false;
        }
    } else if (right_copy.kind == AK_InterpolatedString) {
                                            ...
                                            u32*           out_node)
{
    ...
    for (;;) {
        ...
        if (state->token.kind == TK_InterpolatedStringEnd) {
            ...
            if (start_token.kind == TK_StringContinuationStart &&
                array_count(parts) == 1 && !parts[0].is_expr) {
                AstNode node = {
                    .kind        = AK_StringLiteral,
                             ...
                             u32*           out_node)
{
    *out_unconditional = false;
    ...
    if (state->token.kind != TK_else) {
        if (ast_expr_cursor_kind(state) == TK_else) {
            if (!ast_next_token(state)) {
                return false;
            }
        } else if (ast_peek_kind_at(state, 1) == TK_else) {
            if (!ast_next_token(state) || !ast_next_token(state)) {
                return false;
            }
        }
    ...
    if (state->token.kind == TK_else) {
        ...
        u32 false_payload = 0;
        if (!ast_parse_on_branch_expr(state, &false_payload)) {
            return false;
        }
        ...
        u32 true_pattern = (u32)array_count(state->patterns);
...
internal bool ast_parse_on_branch_expr(AstParseState* state, u32* out_node)
{
    ...
    if (state->token.kind == TK_break || state->token.kind == TK_again) {
        ...
        if (kind == AK_BreakExpr &&
            ...
            ast_expr_cursor_kind(state) == TK_on) {
            if (!ast_next_token(state)) {
                return false;
            }
            bool unconditional = false;
            return ast_parse_break_on_expr(
                state, token_index, label, &unconditional, out_node);
        }
        if (kind == AK_BreakExpr &&
            !ast_token_has_newline_before(state, state->token_index) &&
            ast_token_starts_expression(ast_expr_cursor_kind(state))) {
...
internal bool ast_parse_nud(AstParseState* state, AstToken token, u32* out_node)
{
    ...
    case TK_Caret:
        {
            ...
            AstNode node = {
                .kind        = token.kind == TK_Bang    ? AK_LogicalNot
                               : token.kind == TK_Tilde ? AK_BitwiseNot
    ...
    case TK_At:
        return ast_parse_builtin_macro(state, token, out_node);
    default:
        return error_0201_missing_value(
...
bool ast_parse_expr_bp(AstParseState* state, u8 min_bp, u32* out_node)
{
    ...
    for (;;) {
        ...
        if (!caret_is_postfix &&
            ...
            if (next.kind == TK_StringContinuationStart &&
                ...
                if (!ast_parse_interpolated_string(
                        state, state->token, &right_node)) {
                    return false;
                }

                if (state->nodes[left_node].kind == AK_InterpolatedString) {
                    if (!ast_emit_interpolated_string_continuation(
                            state, next, left_node, right_node, &left_node)) {
                        return false;
                    }
                    continue;
                }
