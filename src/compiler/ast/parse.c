...
internal bool ast_skip_until_matching_rbracket(const AstParseState* state,
                                               u32*                 io_index)
{
    u32 paren_depth   = 0;
    u32 bracket_depth = 0;
...
internal bool ast_skip_enum_value_tokens(const AstParseState* state,
                                         u32*                 io_index)
{
                                         ...
                                         ErrorSpan*           out_span)
{
    ...
    for (;;) {
        ...
        if (kind == TK_Colon || kind == TK_Equal) {
            ErrorSpan start =
                ast_span_for_token_index(state, state->token.token_index);
            ErrorSpan end = ast_span_for_token_index(state, last_value);
            *out_span     = (ErrorSpan){.start = start.start, .end = end.end};
            return true;
        }
...
internal bool ast_skip_type_tokens(const AstParseState* state, u32* io_index)
{
    ...
    if (kind == TK_plex || kind == TK_union) {
        (*io_index)++;
        if (ast_kind_at_stream_index(state, *io_index) == TK_LBracket) {
            (*io_index)++;
            if (ast_kind_at_stream_index(state, *io_index) != TK_RBracket &&
                !ast_skip_until_matching_rbracket(state, io_index)) {
                return false;
            }
            if (ast_kind_at_stream_index(state, *io_index) != TK_RBracket) {
                return false;
    ...
    if (kind == TK_enum) {
        ...
        while (ast_kind_at_stream_index(state, *io_index) != TK_RBrace) {
            ...
            if (ast_kind_at_stream_index(state, *io_index) == TK_Equal) {
                (*io_index)++;
...
internal bool ast_parse_optional_generic_params(AstParseState* state,
                                                u32* out_generic_params_index)
{
    *out_generic_params_index = U32_MAX;
    ...
    if (state->token.kind != TK_LBracket &&
        !ast_expect_token(state, TK_LBracket)) {
        return false;
    }
    ...
    *out_generic_params_index = index;
    return true;
}
...
bool ast_parse_type(AstParseState* state, u32* out_node)
{
    ...
    if (state->token.kind == TK_Symbol) {
        ...
        while (ast_peek_kind_at(state, 0) == TK_LBracket) {
            ...
            if (!ast_expect_token(state, TK_LBracket) ||
                !ast_next_token(state)) {
                return false;
            }

            u32 first_arg = (u32)array_count(state->tuple_items);
            u32 arg_count = 0;
            if (state->token.kind != TK_RBracket) {
                for (;;) {
                    u32 arg_node = 0;
                    if (!ast_parse_type(state, &arg_node)) {
                        return false;
                    }
                    array_push(state->tuple_items, arg_node);
                    arg_count++;
            ...
            u32 apply_index = (u32)array_count(state->type_applications);
            array_push(state->type_applications,
                       (AstTypeApplyInfo){
            ...
            if (!ast_emit_node(state,
                               (AstNode){
                                   .kind        = AK_TypeApply,
                                   .token_index = lbracket_token_index,
                                   .a           = apply_index,
                               },
                               &target_node)) {
                return false;
    ...
    if (state->token.kind == TK_enum) {
        ...
        while (state->token.kind != TK_RBrace) {
            ...
            if (ast_peek_kind_at(state, 0) == TK_Equal) {
                ...
                bool previous_boundary = state->allow_statement_boundary;
                state->allow_statement_boundary = true;
                bool parsed = ast_parse_expr(state, &variant_value_node);
                state->allow_statement_boundary = previous_boundary;
                if (!parsed) {
                    return false;
                }
...
internal bool ast_parse_trait(AstParseState* state, u32* out_node)
{
    ASSERT(state->token.kind == TK_trait, "Expected 'trait' token");
    AstToken trait_token = state->token;
    ...
    while (state->token.kind != TK_RBrace) {
        ...
        AstToken member_token = state->token;
        ...
        if (!ast_next_token(state)) {
            return error_0201_missing_value(
                state->lexer->source,
                ast_token_span(state, &member_token),
                TK_fn);
        }

        u32 value_node = 0;
        if (!ast_parse_type(state, &value_node)) {
            return false;
        }
        if (state->nodes[value_node].kind != AK_TypeFn) {
            return error_0203_expected_token(
                state->lexer->source,
                ast_token_span(state, &state->token),
                TK_fn,
        ...
        if (!ast_emit_node(state,
                           (AstNode){
                               .kind        = AK_Bind,
                               .token_index = member_token.token_index,
                               .a           = member_token.value.symbol_handle,
                               .b           = value_node,
                           },
                           NULL)) {
            return false;
...
bool ast_parse_enum_variant_pattern(AstParseState* state, u32* out_pattern)
{
    ...
    if (state->token.kind == TK_LBrace) {
        braced_payload = true;
    } else if (state->token.kind != TK_LParen) {
        return error_0203_expected_token(state->lexer->source,
    ...
    if (braced_payload) {
        ...
        if (!ast_parse_plex_pattern(state, U32_MAX, &payload_pattern)) {
            return false;
        }
        array_push(state->pattern_items, payload_pattern);
        pattern_count = 1;
    } else {
        if (!ast_next_token(state)) {
            return false;
...
bool ast_parse_for(AstParseState* state, u32* out_node)
{
    ...
    if (!ast_for_token_is_body_start(state->token.kind) &&
        ...
        if (ast_cursor_kind(state) == TK_Comma) {
            ...
            for_info.index_token_index = state->token.token_index;
            if (!ast_next_token(state) || state->token.kind != TK_Comma ||
                !ast_next_token(state)) {
                return error_0203_expected_token(
                    state->lexer->source,
                    ast_token_span(state, &state->token),
                           ...
                           AstNodeFlag    flags)
{
    ...
    if (state->token.kind == TK_trait) {
        UNUSED(allow_ffi_block);
        return ast_parse_trait(state, out_node);
    }
