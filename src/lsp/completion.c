...
internal bool
lsp_completion_add_enum_payload_literal_fields_for_type(Arena*     arena,
                                                        JsonValue* items,
                                                        const LspDocument* doc,
                                                        u32    enum_type_index,
                                                        string variant_name,
                                                        Array(string) seen)
{
    const Lexer* lexer        = &doc->front_end.lexer;
    const Sema*  sema         = &doc->front_end.sema;
    enum_type_index           = sema_materialise_type(sema, enum_type_index);
    const SemaType* enum_type = NULL;
    if (!lsp_sema_type(sema, enum_type_index, &enum_type) ||
        enum_type->kind != STK_Enum) {
        return false;
    }

    for (u32 variant = 0; variant < enum_type->param_count; ++variant) {
        u32 param_index = enum_type->first_param_type + variant;
        u32 symbol      = sema->type_param_symbols[param_index];
        if (symbol == U32_MAX ||
            !string_eq(lex_symbol(lexer, symbol), variant_name)) {
            continue;
        }

        u32 payload_type_index =
            sema_materialise_type(sema, sema->type_param_types[param_index]);
        const SemaType* payload_type = NULL;
        if (!lsp_sema_type(sema, payload_type_index, &payload_type) ||
            payload_type->kind != STK_Plex ||
            !sema->type_param_braced_payloads[param_index]) {
            return false;
        }

        usize before = array_count(items->array.values);
        for (u32 field = 0; field < payload_type->param_count; ++field) {
            u32 field_symbol =
                sema->type_param_symbols[payload_type->first_param_type +
                                         field];
            if (field_symbol == U32_MAX) {
                continue;
            }
            string field_name = lex_symbol(lexer, field_symbol);
            if (!lsp_completion_seen_name(seen, field_name)) {
                lsp_completion_add_plex_literal_field(arena, items, field_name);
            }
        }
        return array_count(items->array.values) > before;
    }
...
internal bool
lsp_completion_add_ast_enum_payload_literal_fields(Arena*       arena,
                                                   JsonValue*   items,
                                                   const Lexer* lexer,
                                                   const Ast*   ast,
                                                   string       enum_name,
                                                   ...
                                                   Array(string) seen)
{
    const AstNode* enum_value = NULL;
    for (u32 i = 0; i < array_count(ast->nodes); ++i) {
        const AstNode* node = &ast->nodes[i];
        if (node->kind == AK_Bind && node->a != U32_MAX &&
            string_eq(lex_symbol(lexer, node->a), enum_name) &&
            node->b < array_count(ast->nodes)) {
            enum_value = &ast->nodes[node->b];
            break;
        }
    }
    if (enum_value == NULL || enum_value->kind != AK_TypeEnum ||
        enum_value->a >= array_count(ast->enum_types)) {
        return false;
    }

    const AstNode*         payload   = NULL;
    const AstEnumTypeInfo* enum_type = &ast->enum_types[enum_value->a];
    for (u32 i = 0; i < enum_type->variant_count; ++i) {
        const AstEnumVariant* variant =
            &ast->enum_variants[enum_type->first_variant + i];
        if (variant->braced_payload && variant->symbol_handle != U32_MAX &&
            string_eq(lex_symbol(lexer, variant->symbol_handle),
                      variant_name) &&
            variant->type_node_index < array_count(ast->nodes)) {
            payload = &ast->nodes[variant->type_node_index];
            break;
        }
    }
    if (payload == NULL || payload->kind != AK_TypePlex ||
        payload->a >= array_count(ast->plex_types)) {
        return false;
    }
    usize                  before = array_count(items->array.values);
    const AstPlexTypeInfo* plex   = &ast->plex_types[payload->a];
    for (u32 i = 0; i < plex->field_count; ++i) {
        const AstPlexField* field = &ast->plex_fields[plex->first_field + i];
        if (field->symbol_handle != U32_MAX) {
            string name = lex_symbol(lexer, field->symbol_handle);
...
internal bool
lsp_completion_add_enum_payload_literal_fields_from_file(Arena*     arena,
                                                         JsonValue* items,
                                                         cstr       path,
                                                         string     enum_name,
                                                         string variant_name,
                                                         Array(string) seen)
{
    ...
    if (lex((NerdSource){.source = source, .source_path = s(path)}, &lexer)) {
        Ast ast = ast_parse(&lexer);
        added   = lsp_completion_add_ast_enum_payload_literal_fields(
            arena, items, &lexer, &ast, enum_name, variant_name, seen);
        ast_done(&ast);
    }
    lex_done(&lexer);
    error_system_set_mode(previous_mode);
...
internal bool
lsp_completion_add_enum_payload_literal_fields(Arena*             arena,
                                               JsonValue*         items,
                                               const LspDocument* doc,
                                               string             enum_name,
                                               string             variant_name,
                                               Array(string) seen)
{
    const Lexer* lexer = &doc->front_end.lexer;
    const Sema*  sema  = &doc->front_end.sema;
    for (u32 local_index = 0; local_index < array_count(sema->locals);
         ++local_index) {
        const SemaLocal* local = &sema->locals[local_index];
        if (local->kind == SLK_TypeAlias && local->symbol_handle != U32_MAX &&
            string_eq(lex_symbol(lexer, local->symbol_handle), enum_name) &&
            lsp_completion_add_enum_payload_literal_fields_for_type(
                arena, items, doc, local->type_index, variant_name, seen)) {
            return true;
        }
    }
    ...
    for (u32 decl_index = 0; decl_index < array_count(sema->decls);
        ...
        if (decl->kind != SK_TypeAlias || decl->symbol_handle == U32_MAX ||
            !string_eq(lex_symbol(lexer, decl->symbol_handle), enum_name)) {
            continue;
        }

        if (lsp_completion_add_enum_payload_literal_fields_for_type(
                arena, items, doc, decl->type_index, variant_name, seen)) {
            return true;
        }
    }
    return false;
...
internal bool
lsp_completion_add_enum_payload_literal_fields_from_uses(Arena*     arena,
                                                         JsonValue* items,
                                                         const LspDocument* doc,
                                                         string             uri,
                                                         string enum_name,
    ...
    while (line_start < doc->source.count) {
        ...
        if (module_path.count > 0 &&
            lsp_completion_resolve_text_module(
                &temp, doc, module_path, uri, &resolved)) {
            bool added =
                lsp_completion_add_enum_payload_literal_fields_from_file(
                    arena, items, resolved, enum_name, variant_name, seen);
            if (!added && string_eq(path_filename(s(resolved)), s("mod.n"))) {
                cstr    module_dir = path_dirname(&temp, resolved);
                DirIter iter       = {0};
                if (dir_iter_init(&iter, module_dir)) {
                    cstr path         = NULL;
                                                     ...
                                                     usize              offset)
{
    ...
    if (module_name.count > 0 &&
        lsp_completion_add_enum_payload_literal_fields(
            arena, items, doc, module_name, type_name, seen)) {
        array_free(seen);
        return true;
    }
    if (module_name.count > 0 &&
        lsp_completion_add_enum_payload_literal_fields_from_uses(
            arena, items, doc, uri, module_name, type_name, seen)) {
        array_free(seen);
        return true;
    }
