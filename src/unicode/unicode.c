//------------------------------------------------------------------------------
// Unicode helpers implementation
//
...
internal bool unicode_is_in_intervals(u32 cp, const UnicodeInterval* intervals, usize count)
{
    usize low  = 0;
    usize high = count;

    while (low < high) {
        usize mid = low + ((high - low) >> 1);
        u32   lo  = intervals[mid].first;
        u32   hi  = intervals[mid].last;
...
internal bool unicode_is_continuation(u8 byte) { return (byte & 0xC0u) == 0x80u; }

internal usize unicode_utf8_decode_first(const u8* data, usize count, u32* out_cp)
{
    if (!data || count == 0 || !out_cp) {
        if (out_cp) {
            *out_cp = 0xFFFDu;
        }
        return 0;
    ...
    if (b0 < 0xE0u) {
        if (count < 2 || !unicode_is_continuation(data[1])) {
            *out_cp = 0xFFFDu;
            return 1;
        }
        *out_cp = ((u32)(b0 & 0x1Fu) << 6) | (u32)(data[1] & 0x3Fu);
        return 2;
    }

    if (b0 < 0xF0u) {
        if (count < 3 || !unicode_is_continuation(data[1]) ||
            !unicode_is_continuation(data[2])) {
            *out_cp = 0xFFFDu;
            return 1;
        }
        ...
        *out_cp = ((u32)(b0 & 0x0Fu) << 12) | ((u32)(data[1] & 0x3Fu) << 6) |
                  (u32)(data[2] & 0x3Fu);
        return 3;
    }

    if (b0 < 0xF5u) {
        if (count < 4 || !unicode_is_continuation(data[1]) ||
            !unicode_is_continuation(data[2]) ||
            !unicode_is_continuation(data[3])) {
            *out_cp = 0xFFFDu;
            return 1;
        }
        ...
        *out_cp = ((u32)(b0 & 0x07u) << 18) | ((u32)(data[1] & 0x3Fu) << 12) |
                  ((u32)(data[2] & 0x3Fu) << 6) | (u32)(data[3] & 0x3Fu);
        return 4;
    }
...
internal usize unicode_codepoint_cell_width(u32 cp)
{
    static const UnicodeInterval combining[] = {
        ...
        {0x11EF3, 0x11EF4}, {0x13430, 0x13440}, {0x13447, 0x13455},
        ...
        {0x1E2EC, 0x1E2EF}, {0x1E4EC, 0x1E4EF}, {0x1E8D0, 0x1E8D6},
        {0x1E944, 0x1E94A}, {0x1F3FB, 0x1F3FF}, {0xE0001, 0xE0001},
        {0xE0020, 0xE007F}, {0xE0100, 0xE01EF},
    };
    static const UnicodeInterval wide[] = {
        ...
        {0x3000, 0x303E}, {0x3041, 0x33FF}, {0x3400, 0x4DBF}, {0x4E00, 0xA4C6},
        {0xA960, 0xA97C}, {0xAC00, 0xD7A3}, {0xF900, 0xFAFF}, {0xFE10, 0xFE19},
        {0xFE30, 0xFE6B}, {0xFF01, 0xFF60}, {0xFFE0, 0xFFE6}, {0x1F004, 0x1F004},
    ...
    if (unicode_is_in_intervals(
            cp, combining, sizeof(combining) / sizeof(combining[0]))) {
        return 0;
    }
    if (unicode_is_in_intervals(cp, wide, sizeof(wide) / sizeof(wide[0]))) {
        return 2;
    }
    return 1;
}
...
usize unicode_utf8_char_cell_width(string text, usize* bytes_consumed)
{
    ...
    u32   cp       = 0;
    usize consumed = unicode_utf8_decode_first(text.data, text.count, &cp);
    if (consumed == 0) {
        consumed = 1;
    }
    if (bytes_consumed) {
    ...
    return unicode_codepoint_cell_width(cp);
}
...
usize unicode_utf8_string_cell_width(string text)
{
    ...
    while (i < text.count) {
        ...
        usize consumed = 0;
        width += unicode_utf8_char_cell_width(rest, &consumed);
