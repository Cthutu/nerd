...
| Part 2: values/bindings | `::`, `:=`, typed locals, assignment, compound assignment, `undefined`, unused locals | Covered. Uninitialised/default locals lower to explicit `default` HIR. | `tests/hir/004-locals.hir`, `009-assignments.hir`, language `015`, `085`, `109` |
...
| Part 9: dynamic arrays | type syntax, push/pop/reserve/clear/free, slicing, ownership with `defer`, returning arrays | Covered. Default dynamic-array locals lower to explicit `default` HIR and LLVM still emits the runtime null/default header state. | language `099`, `101`, commands `068`-`076`, `081` |
...
The removed default-value placeholders are now represented as `default`
expressions, leaving module values and declaration-only type forms as the
remaining accepted-code unsupported cases.
...
1. Add focused HIR snapshots for anonymous entity/binding cases:
   ...
   module bindings.
2. Represent module values and local type declarations explicitly enough that
   they do not appear as unsupported expressions in accepted code.
3. Add focused HIR/LLVM tests for expression-valued `on`/loop control cases
   that currently rely on language fixtures only.
4. Add focused HIR tests for dynamic-array default values, pointer auto-deref,
