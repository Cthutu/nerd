...
M11 passes, omitted `--jobs` remains one; `--jobs auto` applies the measured opt-in
dispatch policy. The Linux experiment did not meet the adoption target; this is
not permission to switch the default or claim M11 acceptance. M6 parallel C emission remains
...
Linux calibration retained 16 KiB of source / 2,048 AST nodes / 1,024 HIR nodes
as experimental grains. It includes tiny, real, wide/deep and dominant-module
inputs, two candidate policies, scaled synthetic inputs and an unrestricted
32-logical-CPU repeat. No universally optimal crossover was established. Worker
reuse was evaluated against measured dispatch/drain costs and is not introduced
without evidence that its added lifetime complexity pays off. The current policy
remains opt-in; neither a guaranteed speedup nor a memory limit is claimed.
