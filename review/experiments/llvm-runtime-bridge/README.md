...
- compiles a tiny C wrapper around `data/epilogue.c` to LLVM IR with clang
...
- The epilogue wrapper only adds prototypes for `init` and `$main`; it still
  includes the real `data/epilogue.c`.
