...
bind main = fn.0
func fn.0() -> i32 {
  let null_ptr: ^u8 = ^u8 nil
  let display_addr: untyped integer = untyped integer 4096
  let display: ^void = ^void cast(untyped integer local.0(display_addr) as ^void)
  let value: usize = usize 8192
  let other: ^u8 = ^u8 cast(usize local.3(value) as ^u8)
  ...
  expr void on bool equal(^u8 local.4(other), ^u8 nil) {
    value(bool yes) => {
      return i32 3
    }
  }
