#!/usr/bin/env python3
"""Exercise persistent work-stealing pools and dependency graphs."""
import argparse
import os
    ...
    with tempfile.TemporaryDirectory(prefix='nerd-threads-') as directory:
        for release, injected in [(False, False), (True, False),
            ...
            command = cc + flags + defines + [
                str(ROOT / 'tests/core/task-graph.c'),
                str(ROOT / 'src/core/mutex.c'),
                str(ROOT / 'src/core/task_graph.c'),
            ...
            for _ in range(3):
                ...
                assert result.stdout == 'task-graph ok\n', result
    print('Dependency ordering, reuse, failure drain and partial pool startup passed')
